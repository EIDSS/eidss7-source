	...
	public static class CoreHtmlHelpers
	{
		public static HtmlString RenderReportViewer(this IHtmlHelper helper, ReportViewerModel model, int? startPage = 1)
		{
			...
			{ 
				...
				sb.AppendLine($"							<a  title='First Page' class='btn btn-default FirstPage'{(contentData.TotalPages == 1 ? " disabled='disabled'" : "")}><span><i class='fas fa-angle-double-left'></i></span></a>");
				sb.AppendLine($"							<a  title='Previous Page' class='btn btn-default PreviousPage'{(contentData.TotalPages == 1 ? " disabled='disabled'" : "")}><span class='fas fa-angle-left'></span></a>");
				sb.AppendLine("						</div>");
				sb.AppendLine("						<div class='btn-group'>");
				...
				sb.AppendLine("						</div>");
				sb.AppendLine("						<div class='btn-group'>");
				sb.AppendLine($"							<a  title='Next Page' class='btn btn-default NextPage'{(contentData.TotalPages == 1 ? " disabled='disabled'" : "")}><span class='fas fa-angle-right'></span></a>");
				sb.AppendLine($"							<a title='Last Page' class='btn btn-default LastPage'{(contentData.TotalPages == 1 ? " disabled='disabled'" : "")}><span class='fas fa-angle-double-right'></span></a>");
				sb.AppendLine("						</div>");
			}
			sb.AppendLine("						<div class='btn-group'>");
			...
			sb.AppendLine($"								<a  title='Find' class='btn btn-info FindTextButton'><span class='fas fa-search' style='padding-right: .5em;'></span>Find</a>");
			sb.AppendLine("							</span>");
			sb.AppendLine("						</div>");
			sb.AppendLine("						<div class='btn-group'>");
			sb.AppendLine("							<a  title='Export' class='dropdown-toggle btn btn-default' data-toggle='dropdown' role='button' aria-haspopup='true' area-expanded='false'>");
			sb.AppendLine("								<span style='color: steelblue;'><i class='fas fa-save'></i></span>");
			sb.AppendLine("								<span class='caret'></span>");
			sb.AppendLine("							</a>");
			sb.AppendLine("							<ul class='dropdown-menu'>");
			sb.AppendLine("								<li><a class='ExportCsv' style='cursor:pointer'>CSV comma delimited)</a></li>");
			sb.AppendLine("								<li><a  class='ExportExcelOpenXml' style='cursor:pointer'>Excel</a></li>");
			sb.AppendLine("								<li><a class='ExportMhtml' style='cursor:pointer'>MHTML (web archive)</a></li>");
			sb.AppendLine("								<li><a class='ExportPdf' style='cursor:pointer'>PDF</a></li>");
			sb.AppendLine("								<li><a  class='ExportTiff' style='cursor:pointer'>TIFF file</a></li>");
			...
			Console.Write("RenderReportViewer end " + DateTime.Now.ToString());
