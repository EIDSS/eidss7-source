...
namespace EIDSS.Web.Components.Veterinary.SearchAggregateDiseaseReport
{
    public class SearchAggregateDiseaseReportBase : SearchComponentBase<SearchAggregateDiseaseReportPageViewModel>, IDisposable
    {
        #region Globals
        ...
        [Inject] private ILogger<SearchAggregateDiseaseReportBase> Logger { get; set; }
        ...
        protected SearchAggregateDiseaseReportPageViewModel Model { get; set; }
        protected RadzenTemplateForm<SearchAggregateDiseaseReportPageViewModel> Form { get; set; }
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            var indicatorResult =
                await BrowserStorage.GetAsync<bool>(SearchPersistenceKeys
                    .VeterinaryAggregateDiseaseReportSearchPerformedIndicatorKey);
            var searchPerformedIndicator = indicatorResult is { Success: true, Value: true };
            if (searchPerformedIndicator)
            {
                var searchModelResult =
                    await BrowserStorage.GetAsync<SearchAggregateDiseaseReportPageViewModel>(SearchPersistenceKeys
                        .VeterinaryAggregateDiseaseReportSearchModelKey);
                ...
                if (searchModel != null)
                {
                    ...
                    if (RefreshResultsIndicator)
                    {
                        ...
                        await BrowserStorage.SetAsync(
                            SearchPersistenceKeys.VeterinaryAggregateDiseaseReportSearchPerformedIndicatorKey, true);
                        await BrowserStorage.SetAsync(SearchPersistenceKeys.VeterinaryAggregateDiseaseReportSearchModelKey,
                            Model);
                    }
                    else
                    {
            ...
            if (searchSubmitted)
            {
                try
                {
                    ...
                    var request = new AggregateReportSearchRequestModel
                    {
                        ...
                        AggregateReportTypeID = (long)AggregateDiseaseReportTypes.VeterinaryAggregateDiseaseReport
                    };
                    ...
                    //sorting
                    request.SortColumn = !IsNullOrEmpty(args.OrderBy) ? args.OrderBy : "ReportID";
        ...
        /// <returns></returns>
        protected async Task HandleValidSearchSubmit(SearchAggregateDiseaseReportPageViewModel model)
        {
            if (Form.IsValid && HasCriteria(model))
            {
                var userPermissions = GetUserPermissions(PagePermission.AccessToVeterinaryAggregateDiseaseReports);
        ...
        protected async Task OpenEdit(AggregateReportGetListViewModel model)
        {
            ...
            await BrowserStorage.SetAsync(SearchPersistenceKeys.VeterinaryAggregateDiseaseReportSearchPerformedIndicatorKey,
                true);
            await BrowserStorage.SetAsync(SearchPersistenceKeys.VeterinaryAggregateDiseaseReportSearchModelKey, Model);

            shouldRender = false;
            const string path = "Veterinary/VeterinaryAggregateDiseasesReport/Details";
        ...
        protected async Task OpenAdd()
        {
            ...
            if (Model.SearchResults != null)
                await BrowserStorage.SetAsync(
                    SearchPersistenceKeys.VeterinaryAggregateDiseaseReportSearchPerformedIndicatorKey,
                    true);
            await BrowserStorage.SetAsync(SearchPersistenceKeys.VeterinaryAggregateDiseaseReportSearchModelKey, Model);
        ...
        private async Task SetDefaults()
        {
            ...
            if (aggregateSettings != null && aggregateSettings.Any())
                defaultSettings = aggregateSettings.FirstOrDefault(x =>
                    x.idfsAggrCaseType == Convert.ToInt64(AggregateValue.Veterinary));
        ...
        private void InitializeModel()
        {
            ...
            Model = new SearchAggregateDiseaseReportPageViewModel
            {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool HasCriteria(SearchAggregateDiseaseReportPageViewModel model)
        {
        ...
        protected async Task PrintSearchResults()
        {
            if (Form.IsValid)
                try
                {
                    ...
                    reportModel.AddParameter("ReportTitle", Localizer.GetString(HeadingResourceKeyConstants.VeterinaryAggregateDiseaseReportPageHeading));
                    reportModel.AddParameter("PersonID", authenticatedUser.PersonId);
                    reportModel.AddParameter("AggregateReportTypeID", ((long)AggregateDiseaseReportTypes.VeterinaryAggregateDiseaseReport).ToString());
                    ...
                    await DiagService.OpenAsync<DisplayReport>(
                        Localizer.GetString(HeadingResourceKeyConstants.VeterinaryAggregateDiseaseReportPageHeading),
                        new Dictionary<string, object>
                            {{"ReportName", "SearchForVeterinaryAggregateDiseasesReport"}, {"Parameters", reportModel.Parameters}},
                        new DialogOptions
                        {
