...
public class DiseaseReportSampleBase : BaseComponent
{
    ...
    protected bool LinkLocalSampleIDToReportSessionID;
    ...
    private async Task SetDataInLocalStorageAndHdrContainer()
    {
        ...
        HdrStateContainer.MinimumValueOfSampleCollectionDateFromSamples = Model.SamplesDetails
            .Where(x => x.CollectionDate.HasValue && x.RowAction != (int) RowActionTypeEnum.Delete)
            .Min(x => x.CollectionDate);
        HdrStateContainer.MinimumValueOfSampleSentDateFromSamples = Model.SamplesDetails
            .Where(x => x.SentDate.HasValue && x.RowAction != (int) RowActionTypeEnum.Delete)
            .Min(x => x.SentDate);
    }

    protected async Task OnSamplesCollectedChange(long? value)
    {
        Model.SamplesCollectedYN = value;
        await SetDataInLocalStorageAndHdrContainer();
    }
    ...
    protected async Task OpenAddModal()
    {
        try
        {
            ...
            dialogParams.Add("idfDisease", diseaseId);
            ...
            dynamic result = await DiagService.OpenAsync<DiseaseReportSampleAddModal>(Localizer.GetString(HeadingResourceKeyConstants.HumanDiseaseReportSamplesHeading),
                dialogParams, new DialogOptions() { Width = "80vw", Resizable = true, Draggable = false, Height = "auto" });
            ...
            if (test != null && test.Validate())
            {
                ...
                item.idfDisease = obj.idfDisease;
    ...
    protected async Task OpenEditModal(DiseaseReportSamplePageSampleDetailViewModel data)
    {
        try
        {
            ...
            dynamic result = await DiagService.OpenAsync<DiseaseReportSampleAddModal>(Localizer.GetString(HeadingResourceKeyConstants.HumanDiseaseReportSamplesHeading),
                dialogParams, new DialogOptions() { Width = "80vw", Resizable = true, Draggable = false });
