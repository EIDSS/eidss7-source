...
namespace EIDSS.Web.Components.Administration.Security.SecurityEventLog
{
    public class SearchSecurityEventLogBase: SearchComponentBase<SearchSecurityEventLogBase>,IDisposable
    {
        ...
        [Inject]
        private ISecurityEventClient SecurityEventClient { get; set; }
        ...
        protected RadzenDataGrid<SecurityEventLogGetListViewModel> grid;
        protected RadzenTemplateForm<SecurityEventLogSearchPageViewModel> form;
        protected SecurityEventLogSearchPageViewModel model;
        
        protected IEnumerable<UserModel> userList;
        protected IEnumerable<BaseReferenceViewModel> actionList;
        protected IEnumerable<BaseReferenceViewModel> ProcessTypeList;
        protected IEnumerable<BaseReferenceViewModel> ResultList;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            ProtectedBrowserStorageResult<bool> indicatorResult = await BrowserStorage.GetAsync<bool>(EIDSSConstants.SearchPersistenceKeys.SecurityEventLogSearchPerformedIndicatorKey);
            var searchPerformedIndicator = indicatorResult.Success && indicatorResult.Value;
            if (searchPerformedIndicator)
            {
                ProtectedBrowserStorageResult<SecurityEventLogSearchPageViewModel> searchModelResult = await BrowserStorage.GetAsync<SecurityEventLogSearchPageViewModel>(EIDSSConstants.SearchPersistenceKeys.SecurityEventLogSearchModelKey);
        ...
        private void InitializeModel()
        {
            ...
            model = new()
            {
                SearchCriteria = new SecurityEventLogGetRequestModel(),
                SearchResults = new List<SecurityEventLogGetListViewModel>()
            };
            count = 0;

            model.Permissions = GetUserPermissions(ClientLibrary.Enumerations.PagePermission.AccessToSecurityLog);
            model.SearchCriteria.SortColumn = "strActionDate";
        ...
        private void SetDefaults()
        {
            ...
            model.SearchCriteria.ActionEndDate = DateTime.Today;
            model.SearchCriteria.ActionStartDate = DateTime.Today.AddDays(-systemPreferences.NumberDaysDisplayedByDefault);
        ...
        protected async Task HandleValidSearchSubmit(SecurityEventLogSearchPageViewModel dataModel)
        {
        ...
        private bool HasCriteria(SecurityEventLogSearchPageViewModel dataModel)
        {
        ...
        protected async Task LoadData(LoadDataArgs args)
        {
            try
            {
                ...
                var request = new SecurityEventLogGetRequestModel()
                {
                    //StartDate = DateTime.Now.AddDays(
                    //    Convert.ToInt32(ConfigurationService.SystemPreferences.NumberDaysDisplayedByDefault * -1)),
                    //EndDate = DateTime.Now,
                    ActionStartDate = model.SearchCriteria.ActionStartDate,
                    ActionEndDate = model.SearchCriteria.ActionEndDate,
                    ObjectId = model.SearchCriteria.ObjectId,
                    Action = model.SearchCriteria.Action,
                    ResultType  =model.SearchCriteria.ResultType,
                    Description = string.IsNullOrEmpty(model.SearchCriteria.Description) ? null : model.SearchCriteria.Description,
                    ErrorText = string.IsNullOrEmpty(model.SearchCriteria.ErrorText) ? null: model.SearchCriteria.ErrorText,
                    ProcessId = string.IsNullOrEmpty(model.SearchCriteria.ProcessId) ? null : model.SearchCriteria.ProcessId,
                    ProcessType = model.SearchCriteria.ProcessType,
                ...
                {
                    request.SortColumn = "strActionDate";
                ...
                var result = await SecurityEventClient.GetSecurityEventLogList(request, token);
                if (result != null)
                {
        ...
        protected async Task GetUserListAsync(LoadDataArgs args)
        {
            ...
            if (args.Sorts?.FirstOrDefault() != null)
            {
                userGetListRequestModel.SortColumn = args.Sorts.FirstOrDefault().Property;
                userGetListRequestModel.SortOrder = args.Sorts.FirstOrDefault().SortOrder.Value
                    .ToString().Replace("Ascending", "ASC").Replace("Descending", "DESC");
            }
            else
            {
                userGetListRequestModel.SortColumn = "Name";
                userGetListRequestModel.SortOrder = "DESC";
            }

            // paging
            if (args.Skip is > 0 && searchSubmitted == false)
            {
                userGetListRequestModel.Page = (args.Skip.Value / 10) + 1;
            }
            else
            {
                userGetListRequestModel.Page = 1;
            }

            userGetListRequestModel.PageSize = 10;
        ...
        protected async Task GetUserListAsync1()
        {
            var userGetListRequestModel = new UserGetRequestModel()
            {
                LanguageId = GetCurrentLanguage(),
        ...
        protected async Task GetActionListAsync(LoadDataArgs args)
        {
            actionCount = 0;
            actionList = await CrossCuttingClient.GetBaseReferenceList(GetCurrentLanguage(), EIDSSConstants.BaseReferenceConstants.SecurityAuditAction, null);
            if (actionList != null) actionCount = actionList.Count();

            if (!IsNullOrEmpty(args.Filter))
            {
                actionList = actionList.Where(c => c.Name != null && c.Name.Contains(args.Filter, StringComparison.CurrentCultureIgnoreCase));
            }
        ...
        protected async Task GetProcessTypeListAsync(LoadDataArgs args)
        {
            ProcessTypeList = await CrossCuttingClient.GetBaseReferenceList(GetCurrentLanguage(), EIDSSConstants.BaseReferenceConstants.SecurityAuditProcessType, null);

            if (!IsNullOrEmpty(args.Filter))
                ProcessTypeList = ProcessTypeList.Where(c => c.Name != null && c.Name.Contains(args.Filter, StringComparison.CurrentCultureIgnoreCase));

            await InvokeAsync(StateHasChanged);
        }

        protected async Task GetResultListAsync(LoadDataArgs args)
        {
            ResultList = await CrossCuttingClient.GetBaseReferenceList(GetCurrentLanguage(), EIDSSConstants.BaseReferenceConstants.SecurityAuditResult, null);

            if (!IsNullOrEmpty(args.Filter))
                ResultList = ResultList.Where(c => c.Name != null && c.Name.Contains(args.Filter, StringComparison.CurrentCultureIgnoreCase));
