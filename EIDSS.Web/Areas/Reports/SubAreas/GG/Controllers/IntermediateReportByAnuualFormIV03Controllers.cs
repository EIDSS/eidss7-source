...
namespace EIDSS.Web.Areas.Reports.SubAreas.GG.Controllers
{
    ...
    [SubArea("GG")]
    [Controller]
    public class IntermediateReportByAnuualFormIV03Controller : Reports.Controllers.ReportController
    {
        ...
        public IntermediateReportByAnuualFormIV03Controller(IConfiguration configuration, ICrossCuttingClient crossCuttingClient, ITokenService tokenService, ILogger<IntermediateReportByAnuualFormIV03Controller> logger) :
                base(configuration, tokenService, crossCuttingClient, logger)
        {
            _logger = logger;
            _configuration = configuration;
            _crossCuttingClient = crossCuttingClient;
            ReportFileName = "IntermediateReportByAnuualFormIV03";
        ...
        public async Task<IActionResult> Index()
        {
            ...
            { 
                ...
                string minimumDateString = "2005-01-01";
                string maximumDateString = "2012-12-31";
                DateTime minimumIssueDate = DateTime.Parse(minimumDateString);
                DateTime maximumIssueDate = DateTime.Parse(maximumDateString);

                var intermediateReportByAnuualFormIV03ViewModel = new IntermediateReportByAnuualFormIV03ViewModel()
                {
                ...
                return View(intermediateReportByAnuualFormIV03ViewModel);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(IntermediateReportByAnuualFormIV03ViewModel intermediateReportByAnuualFormIV03ViewModel)
        {
            try
            {
                ...
                if (gisRegionList.Count > 0)
                {
                    if (string.IsNullOrEmpty(Convert.ToString(intermediateReportByAnuualFormIV03ViewModel.RegionId)))
                        ...
                        gisRayonList = await _crossCuttingClient.GetGisLocationChildLevel(GetCurrentLanguage(), Convert.ToString(intermediateReportByAnuualFormIV03ViewModel.RegionId));
                }

                intermediateReportByAnuualFormIV03ViewModel.ReportLanguageModels = reportLanguageList;
                intermediateReportByAnuualFormIV03ViewModel.GisRegionList = gisRegionList;
                intermediateReportByAnuualFormIV03ViewModel.GisRayonList = gisRayonList;
                intermediateReportByAnuualFormIV03ViewModel.ShowIncludeSignature = includeSignaturePermission;
                intermediateReportByAnuualFormIV03ViewModel.ShowUseArchiveData = archivePermission;
                ...
                {
                    intermediateReportByAnuualFormIV03ViewModel.JavascriptToRun = "submitReport();";
                    return View(intermediateReportByAnuualFormIV03ViewModel);
                }
        ...
        [HttpPost]
        public IActionResult GenerateReport([FromBody] IntermediateReportByAnuualFormIV03QueryModel reportQueryModel)
        {
            try
            { 
