...
namespace EIDSS.Web.Areas.Reports.SubAreas.GG.Controllers
{
    ...
    [SubArea("GG")]
    [Controller]
    public class SerologyResearchResultsController : Reports.Controllers.ReportController
    {
        ...
        public SerologyResearchResultsController(IConfiguration configuration, ICrossCuttingClient crossCuttingClient, ITokenService tokenService,  ILogger<SerologyResearchResultsController> logger) : 
                base(configuration, tokenService, crossCuttingClient, logger)
        {
            _logger = logger;
            _configuration = configuration;
            _crossCuttingClient = crossCuttingClient;
            ReportFileName = "SerologyResearchResults";
        ...
        public async Task<IActionResult> Index()
        {
            ...
            { 
                ...
                var serologyResearchResultsViewModel = new SerologyResearchResultsViewModel()
                {
                ...
                return View(serologyResearchResultsViewModel);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(SerologyResearchResultsViewModel serologyResearchResultsViewModel)
        {
            try
            {
                var reportLanguageList = await _crossCuttingClient.GetReportLanguageList(GetCurrentLanguage());
                var reportYearList = await _crossCuttingClient.GetReportYearList();             
                serologyResearchResultsViewModel.ReportLanguageModels = reportLanguageList;
                serologyResearchResultsViewModel.ShowIncludeSignature = includeSignaturePermission;
                serologyResearchResultsViewModel.ShowUseArchiveData = archivePermission;
                ...
                {
                    serologyResearchResultsViewModel.JavascriptToRun = "submitReport();";
                    return View(serologyResearchResultsViewModel);
                }
        ...
        [HttpPost]
        public IActionResult GenerateReport([FromBody] SerologyResearchResultsQueryModel  reportQueryModel)
        {
            try
            { 
