...
namespace EIDSS.Web.Areas.Reports.SubAreas.GG.Controllers
{
    ...
    [SubArea("GG")]
    [Controller]
    public class VeterinaryYearlySituationController : Reports.Controllers.ReportController
    {
        ...
        public VeterinaryYearlySituationController(IConfiguration configuration, ICrossCuttingClient crossCuttingClient, ITokenService tokenService,  ILogger<VeterinaryYearlySituationController> logger) : 
                base(configuration, tokenService, crossCuttingClient, logger)
        {
            _logger = logger;
            _configuration = configuration;
            _crossCuttingClient = crossCuttingClient;
            ReportFileName = "VeterinaryYearlySituation";
        ...
        public async Task<IActionResult> Index()
        {
            ...
            { 
                ...
                var veterinaryYearlySituationViewModel = new VeterinaryYearlySituationViewModel()
                {
                ...
                return View(veterinaryYearlySituationViewModel);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(VeterinaryYearlySituationViewModel veterinaryYearlySituationViewModel)
        {
            try
            {
                var reportLanguageList = await _crossCuttingClient.GetReportLanguageList(GetCurrentLanguage());
                var reportYearList = await _crossCuttingClient.GetReportYearList();
                reportYearList = ReportHelper.GetFilteredYearList(reportYearList, yearList, "desc");
                veterinaryYearlySituationViewModel.ReportLanguageModels = reportLanguageList;
                veterinaryYearlySituationViewModel.ReportYearModels = reportYearList;
                veterinaryYearlySituationViewModel.ShowIncludeSignature = includeSignaturePermission;
                veterinaryYearlySituationViewModel.ShowUseArchiveData = archivePermission;
                ...
                {
                    veterinaryYearlySituationViewModel.JavascriptToRun = "submitReport();";
                    return View(veterinaryYearlySituationViewModel);
                }
        ...
        [HttpPost]
        public IActionResult GenerateReport([FromBody] VeterinaryYearlySituationQueryModel  reportQueryModel)
        {
            try
            { 
