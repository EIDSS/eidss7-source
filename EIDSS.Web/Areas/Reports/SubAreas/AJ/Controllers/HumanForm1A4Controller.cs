...
namespace EIDSS.Web.Areas.Reports.SubAreas.AJ.Controllers
{
    ...
    [SubArea("AJ")]
    [Controller]
    public class HumanForm1A4Controller : Reports.Controllers.ReportController
    {
        ...
        public HumanForm1A4Controller(IConfiguration configuration, ICrossCuttingClient crossCuttingClient, ITokenService tokenService, ILogger<HumanForm1A4Controller> logger) :
                base(configuration, tokenService, crossCuttingClient, logger)
        {
            _logger = logger;
            _configuration = configuration;
            _crossCuttingClient = crossCuttingClient;
            ReportFileName = "HumanFormN1Report_A4";
        ...
        public async Task<IActionResult> Index()
        {
            ...
            { 
                ...
                var humanForm1A4ViewModel = new HumanForm1A4ViewModel()
                {
                ...
                return View(humanForm1A4ViewModel);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(HumanForm1A4ViewModel humanForm1A4ViewModel)
        {
            try
            {
                ...
                if (gisRegionList.Count > 0)
                {
                    if (string.IsNullOrEmpty(Convert.ToString(humanForm1A4ViewModel.RegionId)))
                        ...
                        gisRayonList = await _crossCuttingClient.GetGisLocationChildLevel(GetCurrentLanguage(), Convert.ToString(humanForm1A4ViewModel.RegionId));
                }
                organiztionList = await _crossCuttingClient.GetBaseReferenceList(GetCurrentLanguage(), BaseReferenceConstants.OrganizationName, Convert.ToInt64(AccessoryCodes.AllHACode));

                humanForm1A4ViewModel.ReportLanguageModels = reportLanguageList;
                humanForm1A4ViewModel.ReportYearModels = reportYearList;
                humanForm1A4ViewModel.ReportFromMonthNameModels = reportMonthNameList;
                humanForm1A4ViewModel.ReportToMonthNameModels = reportMonthNameList;
                humanForm1A4ViewModel.GisRegionList = gisRegionList;
                humanForm1A4ViewModel.GisRayonList = gisRayonList;
                humanForm1A4ViewModel.OrganizationList = organiztionList;
                humanForm1A4ViewModel.ShowIncludeSignature = includeSignaturePermission;
                humanForm1A4ViewModel.ShowUseArchiveData = archivePermission;
                ...
                {
                    humanForm1A4ViewModel.JavascriptToRun = "submitReport();";
                    return View(humanForm1A4ViewModel);
                }
        ...
        [HttpPost]
        public IActionResult GenerateReport([FromBody] HumanForm1A4QueryModel reportQueryModel)
        {
            try
            { 
