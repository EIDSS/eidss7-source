...
-- TODO: change path to JSON file here
SELECT @json = BulkColumn
 FROM OPENROWSET(BULK 'D:\TestData\EIDSS7_UIControlTree.json', SINGLE_CLOB) as j


if OBJECT_ID(N'tempdb..#UIControlTree') is not null
	exec sp_executesql N'drop table #UIControlTree'

if OBJECT_ID(N'tempdb..#UIControlTree') is null
create table #UIControlTree
(	id bigint not null identity(1, 1) primary key,
	[name] nvarchar(200) collate Cyrillic_General_CI_AS not null,
	[filePath] nvarchar(400) collate Cyrillic_General_CI_AS not null,
	[itemType] int not null,
	children nvarchar(max) collate Cyrillic_General_CI_AS null,
	childLevel int not null,
	[parentId] bigint null,
	controlMatchIndex bigint null,
	controlId nvarchar(200) collate Cyrillic_General_CI_AS null,
	controlName nvarchar(200) collate Cyrillic_General_CI_AS null,
	controlType nvarchar(200) collate Cyrillic_General_CI_AS null,
	controlCaptionClass nvarchar(200) collate Cyrillic_General_CI_AS null,
	controlCaptionConstant nvarchar(200) collate Cyrillic_General_CI_AS null,
	controlCaptionEn nvarchar(200) collate Cyrillic_General_CI_AS null,
	isControlInGrid bit null,
	controlGridColumnCaptionClass nvarchar(200) collate Cyrillic_General_CI_AS null,
	controlGridColumnCaptionConstant nvarchar(200) collate Cyrillic_General_CI_AS null,
	controlGridColumnCaptionEn nvarchar(200) collate Cyrillic_General_CI_AS null,
	titleSize int null,
	possibleTitleClass nvarchar(200) collate Cyrillic_General_CI_AS null,
	possibleTitleConstant nvarchar(200) collate Cyrillic_General_CI_AS null,
	possibleTitleEn nvarchar(200) collate Cyrillic_General_CI_AS null
)
truncate table #UIControlTree


insert into #UIControlTree
(	[name],
	[filePath],
	[itemType],
	children,
...
SELECT [Name],
    replace([Path], N'D:/Projects/EIDSS7/', N'') as [FilePath],
    [Type],
    children,
	0
FROM OPENJSON(@json) WITH (
    [Name] NVARCHAR(200) '$.Name',
    [Path] NVARCHAR(400) '$.Path',
    [Type] INT '$.Type',
    children NVARCHAR(MAX) '$.Children' AS JSON
    )

declare @childLevel int = 0
declare @GoOn int = 1

while @GoOn > 0 and @childLevel < 1000
begin
	insert into #UIControlTree
	(	[name],
		[filePath],
	...
	SELECT
		child_info.[Name],
		replace(child_info.[Path], N'D:/Projects/EIDSS7/', N'') as [FilePath],
		child_info.[Type],
		child_info.children,
		ct.childLevel + 1,
		ct.id
	from #UIControlTree ct
	CROSS APPLY OPENJSON(ct.children) WITH 
	(	[Name] NVARCHAR(200) '$.Name',
		[Path] NVARCHAR(400) '$.Path',
		[Type] INT '$.Type',
		children NVARCHAR(MAX) '$.Children' AS JSON
	) child_info
	where ct.childLevel = @childLevel
...
update	ct
set			ct.controlType = ct.[name]
from		#UIControlTree ct
where		ct.itemType = 3
			and ct.controlType is null

update	ct
set			ct.controlId = attr_id.strVal
from		#UIControlTree ct
outer apply
(	select top 1 replace(ct_child.[name], N'id: ', N'') as strVal
	from	#UIControlTree ct_child
	where	ct_child.[parentId] = ct.[id]
			and ct_child.itemType = 4
			and ct_child.[name] like N'id: %' collate Cyrillic_General_CI_AS
) attr_id
where		ct.itemType = 3
			and ct.controlId is null

update	ct
set			ct.controlName = attr_name.strVal
from		#UIControlTree ct
outer apply
(	select top 1 replace(ct_child.[name], N'Name: ', N'') as strVal
	from	#UIControlTree ct_child
	where	ct_child.[parentId] = ct.[id]
			and ct_child.itemType = 4
			and ct_child.[name] like N'Name: %' collate Cyrillic_General_CI_AS
) attr_name
where		ct.itemType = 3
			and ct.controlName is null

update	ct
set			ct.controlMatchIndex = case when isnumeric(attr_ind.strVal) = 1 then CAST(attr_ind.strVal as int) else null end
from		#UIControlTree ct
outer apply
(	select top 1 replace(ct_child.[name], N'Match Index: ', N'') as strVal
	from	#UIControlTree ct_child
	where	ct_child.[parentId] = ct.[id]
			and ct_child.itemType = 4
			and ct_child.[name] like N'Match Index: %' collate Cyrillic_General_CI_AS
) attr_ind
where		ct.itemType = 3
			and ct.controlMatchIndex is null

update	ct
set			ct.controlCaptionClass = case when attr_label.strVal like N'%.%' collate Cyrillic_General_CI_AS then LEFT(attr_label.strVal, CHARINDEX(N'.', attr_label.strVal, 0) - 1) else null end,
			ct.controlCaptionConstant = case when attr_label.strVal like N'%.%' collate Cyrillic_General_CI_AS then RIGHT(attr_label.strVal, LEN(attr_label.strVal) - CHARINDEX(N'.', attr_label.strVal, 0)) else attr_label.strVal end
from		#UIControlTree ct
outer apply
(	select top 1 replace(replace(replace(replace(ct_child.[name], N'Label: ', N''), N'Localizer.GetString(', N''), N'@', N''), N')', N'') as strVal
	from	#UIControlTree ct_child
	where	ct_child.[parentId] = ct.[id]
			and ct_child.itemType = 4
			and ct_child.[name] like N'Label: %' collate Cyrillic_General_CI_AS
) attr_label
where		ct.itemType = 3
			and ct.controlCaptionConstant is null
...
from		#UIControlTree ct
outer apply
(	select top 1 replace(replace(ct_child.[name], N'InterfaceEditorKey: ', N''), N'@', N'') as strVal
	from	#UIControlTree ct_child
	where	ct_child.[parentId] = ct.[id]
			and ct_child.itemType = 4
			and ct_child.[name] like N'InterfaceEditorKey: %' collate Cyrillic_General_CI_AS
) attr_label
where		ct.itemType = 3
			and ct.controlCaptionConstant is null
...
update	ct
set			ct.isControlInGrid = case when isnumeric(attr_ind.strVal) = 1 then CAST(attr_ind.strVal as bit) else CAST(0 as bit) end
from		#UIControlTree ct
outer apply
(	select top 1 replace(replace(replace(ct_child.[name], N'InGrid: ', N''), N'true', N'1'), N'false', N'0') as strVal
	from	#UIControlTree ct_child
	where	ct_child.[parentId] = ct.[id]
			and ct_child.itemType = 4
			and ct_child.[name] like N'InGrid: %' collate Cyrillic_General_CI_AS
) attr_ind
where		ct.itemType = 3
			and ct.isControlInGrid is null


update	ct
set			ct.controlGridColumnCaptionClass = case when attr_label.strVal like N'%.%' collate Cyrillic_General_CI_AS then LEFT(attr_label.strVal, CHARINDEX(N'.', attr_label.strVal, 0) - 1) else null end,
			ct.controlGridColumnCaptionConstant = case when attr_label.strVal like N'%.%' collate Cyrillic_General_CI_AS then RIGHT(attr_label.strVal, LEN(attr_label.strVal) - CHARINDEX(N'.', attr_label.strVal, 0)) else attr_label.strVal end
from		#UIControlTree ct
outer apply
(	select top 1 replace(replace(replace(replace(ct_child.[name], N'ColumnTitle: ', N''), N'Localizer.GetString(', N''), N'@', N''), N')', N'') as strVal
	from	#UIControlTree ct_child
	where	ct_child.[parentId] = ct.[id]
			and ct_child.itemType = 4
			and ct_child.[name] like N'ColumnTitle: %' collate Cyrillic_General_CI_AS
) attr_label
where		ct.itemType = 3
			and ct.isControlInGrid = 1
			and ct.controlGridColumnCaptionConstant is null


update	ct
set			ct.controlCaptionClass = 
				case 
					when replace(replace(replace(replace(replace(replace(replace(ct.[name], N'@', N''), N'h1: ', N''), N'h2: ', N''), N'h3: ', N''), N'h4: ', N''), N'h5: ', N''), N'h6: ', N'') like N'%.%' collate Cyrillic_General_CI_AS
						then LEFT(
								replace(replace(replace(replace(replace(replace(replace(ct.[name], N'@', N''), N'h1: ', N''), N'h2: ', N''), N'h3: ', N''), N'h4: ', N''), N'h5: ', N''), N'h6: ', N''), 
								CHARINDEX(N'.', replace(replace(replace(replace(replace(replace(replace(ct.[name], N'@', N''), N'h1: ', N''), N'h2: ', N''), N'h3: ', N''), N'h4: ', N''), N'h5: ', N''), N'h6: ', N''), 0) - 1)
					else null 
				end,
			ct.controlCaptionConstant = 
				case 
					when ct.[name] like N'%.%' collate Cyrillic_General_CI_AS 
						then RIGHT(ct.[name], LEN(ct.[name]) - CHARINDEX(N'.', ct.[name], 0)) 
					else replace(replace(replace(replace(replace(replace(replace(ct.[name], N'@', N''), N'h1: ', N''), N'h2: ', N''), N'h3: ', N''), N'h4: ', N''), N'h5: ', N''), N'h6: ', N'')
				end,
			ct.controlType = 'Page Title/Sub-title',
			ct.titleSize = 
				case 
					when ct.[name] like N'h1: %' collate Cyrillic_General_CI_AS
						then 1
					when ct.[name] like N'h2: %' collate Cyrillic_General_CI_AS
						then 2
					when ct.[name] like N'h3: %' collate Cyrillic_General_CI_AS
						then 3
					when ct.[name] like N'h4: %' collate Cyrillic_General_CI_AS
						then 4
					when ct.[name] like N'h5: %' collate Cyrillic_General_CI_AS
						then 5
					when ct.[name] like N'h6: %' collate Cyrillic_General_CI_AS
						then 6
					else null 
				end
from		#UIControlTree ct
...
update		ct
set			ct.possibleTitleClass = title_detected.controlCaptionClass,
			ct.possibleTitleConstant = title_detected.controlCaptionConstant
from		#UIControlTree ct
outer apply
(	select top 1
				ct_header.controlCaptionClass, ct_header.controlCaptionConstant
	from		#UIControlTree ct_header 
	where		ct_header.[parentId] = ct.[id] and ct_header.itemType = 5 /*Header*/ 
	order by	isnull(ct_header.titleSize, 7) asc, ct_header.[id] asc
) title_detected
where		ct.itemType in (1 /*View*/, 2 /*Component*/)
...
update		ct
set			ct.controlCaptionEn = c.controlCaptionEn
from		#UIControlTree ct
join		#Caption c
on			c.captionClass = ct.controlCaptionClass
			and c.controlCaptionConstant = ct.controlCaptionConstant
where		ct.controlCaptionConstant is not null
			and ct.controlCaptionEn is null

update		ct
set			ct.controlGridColumnCaptionEn = c.controlCaptionEn
from		#UIControlTree ct
join		#Caption c
on			c.captionClass = ct.controlGridColumnCaptionClass
			and c.controlCaptionConstant = ct.controlGridColumnCaptionConstant
where		ct.controlGridColumnCaptionConstant is not null
			and ct.controlGridColumnCaptionEn is null


update		ct
set			ct.possibleTitleEn = N' (Possible title: ' + ISNULL(c.controlCaptionEn, ct.possibleTitleConstant) + N')'
from		#UIControlTree ct
left join	#Caption c
on			c.captionClass = ct.possibleTitleClass
			and c.controlCaptionConstant = ct.possibleTitleConstant
where		ct.possibleTitleConstant is not null
			and ct.possibleTitleEn is null


/*
select *
from #UIControlTree ct
--join #UIControlTree ct_parent
--on ct_parent.[id] = ct.[parentId]
where ct.[name] <> N'InterfaceEditorResource' collate Cyrillic_General_CI_AS
*/



if OBJECT_ID(N'tempdb..#UIControlNamePath') is not null
	exec sp_executesql N'drop table #UIControlNamePath'

if OBJECT_ID(N'tempdb..#UIControlNamePath') is null
create table #UIControlNamePath
(
	itemId bigint not null primary key,
	itemType int not null,
	itemName nvarchar(200) collate Cyrillic_General_CI_AS not null,
	itemFilePath nvarchar(400) collate Cyrillic_General_CI_AS not null,
	strItemNamePath nvarchar(max) collate Cyrillic_General_CI_AS null
)
truncate table #UIControlNamePath


;
with	uiControlLevelTree
		(
			itemId,
			itemType,
			itemName,
...
as	(	select		ct.[id]	as itemId,
					ct.itemType as itemType,
					case 
						when ct.itemType = 1 /*View*/
							then	N'View (page) [' + isnull(ct.[name], N'') + isnull(ct.possibleTitleEn, N'') + N']' collate Cyrillic_General_CI_AS
						when ct.itemType = 2 /*Component*/
							then	N'Component [' + isnull(ct.[name], N'') + isnull(ct.possibleTitleEn, N'') + N']' collate Cyrillic_General_CI_AS
						else isnull(ct.[name], N'')
					end as itemName,
					ct.filePath as itemFilePath,
					ct.itemType as levelType,
					0 as intParentLevel,
					ct.[parentId] as parentLevelId,
					cast(
						case 
							when ct.itemType = 1 /*View*/
								then	N'View (page) [' + isnull(ct.[name], N'') + isnull(ct.possibleTitleEn, N'') + N']' collate Cyrillic_General_CI_AS
						...
						end as nvarchar(max)) collate Cyrillic_General_CI_AS as strLevelItemPath
		from		#UIControlTree ct
		where		ct.itemType in (1 /*View*/, 2 /*Component*/) 
					and ct.[name] <> N'InterfaceEditorResource' collate Cyrillic_General_CI_AS
		union all
		select		uiControlLevelTree.itemId,
					uiControlLevelTree.itemType,
					uiControlLevelTree.itemName,
					uiControlLevelTree.itemFilePath,
					ct.itemType,
					uiControlLevelTree.intParentLevel + 1 as intParentLevel,
					ct.[parentId] as parentLevelId,
					case 
						when ct.itemType = 0 /*Folder*/
							then N'Folder [' + isnull(ct.[filePath], N'') + N']' collate Cyrillic_General_CI_AS
						when ct.itemType = 1 /*View*/
							then	N'View (page) [' + isnull(ct.[name], N'') + isnull(ct.possibleTitleEn, N'') + N']' collate Cyrillic_General_CI_AS
						when ct.itemType = 2 /*Component*/
							then	N'Component [' + isnull(ct.[name], N'') + isnull(ct.possibleTitleEn, N'') + N']' collate Cyrillic_General_CI_AS
						else isnull(ct.[name], N'')
					end + N'>' + uiControlLevelTree.strLevelItemPath collate Cyrillic_General_CI_AS
		from		#UIControlTree ct
		inner join	uiControlLevelTree
		on			uiControlLevelTree.parentlevelId = ct.[id]
		where		(	ct.itemType <> 0
						or	(	ct.itemType = 0
								and uiControlLevelTree.levelType <> 0
							)
					)
	)

insert into	#UIControlNamePath
(	itemId,
	itemType,
	itemName,
	itemFilePath,
...
select		lt.itemId,
			...
			lt.itemFilePath,
			lt.strLevelItemPath
from		uiControlLevelTree lt
where		lt.levelType = 0
order by	lt.[itemId]
...
select --distinct
			...
			lt.itemFilePath as N'Item File Path in Source Code',
			lt.strItemNamePath as N'Item Name Path',
			isnull(ct.controlType, N'') as N'Control Type',
			isnull(cast(ct.controlMatchIndex as nvarchar(20)), N'') as N'Control Index (in Parse Utility)',
			isnull(ct.controlId, N'') as N'Control Id (on page)',
			isnull(ct.controlId, N'') as N'Control Name (on page)',
			coalesce(ct.controlCaptionEn, ct.controlCaptionConstant, N'') as N'Control Caption (on page)',
			case
				when	ct.isControlInGrid = 1
					then	'Yes'
				else	N''
			end as N'Is Control in Grid',
			case
				when	ct.isControlInGrid = 1
					then	coalesce(ct.controlGridColumnCaptionEn, ct.controlGridColumnCaptionConstant, N'')
				else	N''
			end as N'Grid Column Caption'
from		#UIControlNamePath lt
left join	#UIControlTree ct
on			ct.[parentId] = lt.itemId
			and ct.itemType = 3 /*Control*/
order by lt.[itemId], ct.[id]

/*
select ct.[name], ct.[filePath], ct_ind.[name], ct.children
from #UIControlTree ct
join #UIControlTree ct_ind
on ct_ind.parentId = ct.[id]
	and ct_ind.[itemType] = 4
	and ct_ind.[name] like N'Match Index: %' collate Cyrillic_General_CI_AS
where ct.[name] = N'eidss-calendar'
	and not exists (select 1 from #UIControlTree ct_child where ct_child.parentId = ct.[id] and ct_child.[itemType] = 4 and ct_child.[name] like N'Label: %' collate Cyrillic_General_CI_AS)
*/
