...
select		extp.[name] as strExtendedPropertyName,
			...
			N'EXEC sys.sp_addextendedproperty @name=N''' + replace(extp.[name], N'''', N'''''''''') + 
...
N'	[' + c.strColumnName + N']' + 
	case 
		when	c.[is_computed] = 1 and c.[is_persisted] = 1
			then	N' AS ' + c.[computed_col_def] + N' PERSISTED'
		when	c.[is_computed] = 1 and c.[is_persisted] = 0
			then	N' AS ' + c.[computed_col_def]
			...
			c.strDefaultConstraint =
				case
					when	c.[is_computed] = 0 and c.[DF_name] is not null
						then	N'
ALTER TABLE [' + c.strSchemaName + N'].[' + c.strTableName + N'] ADD  CONSTRAINT [' + c.[DF_name] + N']  DEFAULT ' + c.[default_value] + N' FOR [' + c.strColumnName + N']
GO' collate Latin1_General_CI_AS
					else	null
				end
...
' +
N'
CREATE TABLE [' + t.strSchemaName + N'].[' + t.strTableName + N'](' collate Latin1_General_CI_AS,
isnull(colDefinition.strSql, N'') collate Latin1_General_CI_AS,
case
...
N'	[' + c.strColumnName + N'] [' + c.strColumnType + N']' + 
	...
	case 
		when	c.[default_value] is not null
			then	N' DEFAULT ' + c.[default_value] collate Latin1_General_CI_AS
		else	N'' 
	end +
	case 
		when	c.[column_id] < table_max_col.[column_id]
...
select	N'
CREATE TYPE [' + tt.strSchemaName + N'].[' + tt.strTableTypeName + N'] AS TABLE(' collate Latin1_General_CI_AS,
isnull(colDefinition.strSql, N''),
case
