...
select		@cmd_select = @cmd_select + N', case when oa' + replace(cast(eg.idfEmployeeGroup as nvarchar(20)), N'-', N'_') + N'.idfEmployee is not null then ''a'' else '''' end as N''' + replace(r_eg_en.[name], N'''', N'''''') + N'''',
			@cmd_from = @cmd_from + N'
left join	[dbo].[LkupRoleSystemFunctionAccess] oa' + replace(cast(eg.idfEmployeeGroup as nvarchar(20)), N'-', N'_') + N'
on			oa' + replace(cast(eg.idfEmployeeGroup as nvarchar(20)), N'-', N'_') + N'.idfEmployee = ' + cast(eg.idfEmployeeGroup as nvarchar(20)) + N'
			and oa' + replace(cast(eg.idfEmployeeGroup as nvarchar(20)), N'-', N'_') + N'.intRowStatus = 0
			and oa' + replace(cast(eg.idfEmployeeGroup as nvarchar(20)), N'-', N'_') + N'.[SystemFunctionID] = br.idfsBaseReference
			and oa' + replace(cast(eg.idfEmployeeGroup as nvarchar(20)), N'-', N'_') + N'.[SystemFunctionOperationID] = oo.idfsBaseReference'
