-- Examples:
/*
	EXEC [dbo].[DF_HDR_Rule_SiteAddress]
	EXEC [dbo].[DF_HDR_Rule_SiteAddress] @UsePredefinedData=0,@StartDate='20240601'
	EXEC [dbo].[DF_HDR_Rule_SiteAddress] @UsePredefinedData=1
*/
-- ************************************************************************************************
CREATE OR ALTER PROCEDURE [dbo].[DF_HDR_Rule_SiteAddress] 
(
...
begin
	...
	select distinct
				...
				s.idfSiteGroup
	from		#cfrHC hc
	inner join	tflSiteToSiteGroup s_to_sg_rayon with (nolock)
		inner join	tflSiteGroup sg_rayon with (nolock)
		on			sg_rayon.idfSiteGroup = s_to_sg_rayon.idfSiteGroup
					and sg_rayon.idfsRayon is not null
					and sg_rayon.intRowStatus = 0
	on			s_to_sg_rayon.idfsSite = hc.idfsSite
