    ...
    begin
      print	'Script with version ' + @Version + ' has already been applied to the database ' + DB_NAME() + N' on the server ' + @@servername + N'.'
		...
		set		idfsTentativeDiagnosis = idfsFinalDiagnosis,
				datTentativeDiagnosisDate = isnull(datTentativeDiagnosisDate, datFinalDiagnosisDate)
		where	idfsTentativeDiagnosis is null
				and idfsFinalDiagnosis is not null

		update	dbo.tlbHumanCase
		set		idfsFinalDiagnosis = null,
				datFinalDiagnosisDate = null
		where	idfsTentativeDiagnosis is not null
				and idfsFinalDiagnosis is not null
				and idfsTentativeDiagnosis = idfsFinalDiagnosis
