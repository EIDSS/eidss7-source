...
BEGIN
	...
	BEGIN TRY
		SELECT ISNULL(ha.strFirstName, '') + ' ' + ISNULL(ha.strLastName, '') AS PatientFarmOwnerName,
			...
			CASE 
				WHEN hm.datDateofBirth IS NULL OR hm.datDateofBirth = '' THEN ''
				ELSE CASE
					WHEN HumanAgeType.idfsReference = 10042001 THEN CAST(DATEDIFF(DAY,hm.datDateofBirth,hm.datEnteredDate) AS VARCHAR(3))  
					WHEN HumanAgeType.idfsReference = 10042002 THEN CAST(DATEDIFF(MONTH,hm.datDateofBirth,hm.datEnteredDate) AS VARCHAR(3))  
					WHEN HumanAgeType.idfsReference = 10042004 THEN CAST(DATEDIFF(WEEK,hm.datDateofBirth,hm.datEnteredDate) AS VARCHAR(3)) 
					ELSE CAST(DATEDIFF(YEAR,hm.datDateofBirth,hm.datEnteredDate) AS VARCHAR(3)) 
