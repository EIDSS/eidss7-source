...
-- Description:	Returns a string of comma separated penside test names associated with a given disease
--							
-- Author: Ricky Moss
-- Revision History:
...
-- Ricky Moss		02/01/2018	Added active condition
--
-- Test Code:
-- SELECT dbo.FN_REF_PENSIDETESTNAMETODISEASE_GET('en', 6618330000000);
-- ===================================================================================================

CREATE FUNCTION [dbo].[FN_REF_PENSIDETESTNAMETODISEASE_GET]
(
	@LangID NVARCHAR(50),
...
IF @idfsDiagnosis <> 0
BEGIN
	WITH results ([idfsPensideTest],[strPensideTest]) AS
	(
		SELECT 
			ptfdbr.idfsReference, 
			ptfdbr.name 
	...
	SELECT @CSV = STRING_AGG([strPensideTest], ', ')
	FROM results;
