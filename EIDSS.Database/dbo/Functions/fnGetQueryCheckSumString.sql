...
SELECT  dbo.fnGetQueryCheckSumString (49542660000000) --VetCasesWithSamplesAndTestsReport_Both
*/


create   FUNCTION [dbo].[fnGetQueryCheckSumString](
	@idflQuery bigint --##PARAM @@idflQuery - Query ID
)
...
declare 
	...
	from dbo.tasQuerySearchObject
	where idflQuery = @idflQuery 
	union
	select idfQuerySearchObject
	from dbo.tasQuerySearchObject
	where idfParentQuerySearchObject = @idflQuery 
	...
	(
		...
		from tasQueryConditionGroup tqcg
	...
	from tasQuery tq
	where tq.idflQuery = @idflQuery
	
	select @intQuery = @intQuery + count(*)
	from @QueryConditionGroups
	inner join tasQueryConditionGroup tqcg
	...
	from tasQuerySearchFieldCondition tqsfc
