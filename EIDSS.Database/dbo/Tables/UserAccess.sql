CREATE TABLE [dbo].[UserAccess] (
    ...
    [rowguid]                   UNIQUEIDENTIFIER CONSTRAINT [DF_UserAccess_rowguid] DEFAULT (newsequentialid()) ROWGUIDCOL NOT NULL,
...
CREATE TRIGGER [dbo].[TR_UserAccess_I_Delete] on [dbo].[UserAccess]
INSTEAD OF DELETE
NOT FOR REPLICATION
AS
BEGIN
	...
	BEGIN
		...
		WITH cteOnlyDeletedRows([UserAccessUID]) as
		(
			SELECT [UserAccessUID] FROM deleted
			EXCEPT
			SELECT [UserAccessUID] FROM inserted
		)
		...
		FROM dbo.UserAccess as a 
		INNER JOIN cteOnlyDeletedRows as b 
			ON a.[UserAccessUID] = b.[UserAccessUID];
