...
CREATE VIEW [dbo].[Human_Sample_and_Test_RU]
as
select		
...
left join	fnReferenceRepair('ru', 19000064) [ref_sflHC_Outcome] 
on			[ref_sflHC_Outcome].idfsReference = v.[sflHC_Outcome_ID] 
left join	fnReferenceRepair('ru', 19000011) [ref_sflHC_InitialCaseClassification] 
...
left join	fnReferenceRepair('ru', 19000150) [ref_sflHC_ReasonForNotCollectingSample] 
on			[ref_sflHC_ReasonForNotCollectingSample].idfsReference = v.[sflHC_ReasonForNotCollectingSample_ID] 
left join	fnReferenceRepair('ru', 19000100) [ref_sflHC_ClinicalDiagBasis] 
on			[ref_sflHC_ClinicalDiagBasis].idfsReference = v.[sflHC_ClinicalDiagBasis_ID] 
left join	fnReferenceRepair('ru', 19000100) [ref_sflHC_EpiDiagBasis] 
on			[ref_sflHC_EpiDiagBasis].idfsReference = v.[sflHC_EpiDiagBasis_ID] 
left join	fnReferenceRepair('ru', 19000100) [ref_sflHC_LabDiagBasis] 
on			[ref_sflHC_LabDiagBasis].idfsReference = v.[sflHC_LabDiagBasis_ID] 
left join	fnReferenceRepair('ru', 19000042) [ref_sflHC_PatientAgeType] 
on			[ref_sflHC_PatientAgeType].idfsReference = v.[sflHC_PatientAgeType_ID] 
left join	fnReferenceRepair('ru', 19000046) [ref_sflHCSample_CollectedByOffice] 
...
left join	fnReferenceRepair('ru', 19000019) [ref_sflHCTest_Diagnosis] 
on			[ref_sflHCTest_Diagnosis].idfsReference = v.[sflHCTest_Diagnosis_ID] 
left join	fnReferenceRepair('ru', 19000001) [ref_sflHCTest_TestStatus] 
