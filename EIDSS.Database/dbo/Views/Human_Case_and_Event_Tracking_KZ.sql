...
create VIEW [dbo].[Human_Case_and_Event_Tracking_KZ]
as
...
LEFT JOIN fnReferenceRepair('kk', 19000046) [ref_sflHC_ReceivedByOffice] ON 
	[ref_sflHC_ReceivedByOffice].idfsReference = v.[sflHC_ReceivedByOffice_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000046) [ref_sflHC_SentByOffice] ON 
	[ref_sflHC_SentByOffice].idfsReference = v.[sflHC_SentByOffice_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000035) [ref_sflHC_PatientNotificationStatus] ON 
	[ref_sflHC_PatientNotificationStatus].idfsReference = v.[sflHC_PatientNotificationStatus_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000041) [ref_sflHC_HospitalizationStatus] ON 
...
LEFT JOIN fnReferenceRepair('kk', 19000100) [ref_sflHC_Hospitalization] ON 
	[ref_sflHC_Hospitalization].idfsReference = v.[sflHC_Hospitalization_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000149) [ref_sflHC_NonNotifiableDiagnosis] ON 
	[ref_sflHC_NonNotifiableDiagnosis].idfsReference = v.[sflHC_NonNotifiableDiagnosis_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000046) [ref_sflHC_FacilityWherePatientFSC] ON 
	[ref_sflHC_FacilityWherePatientFSC].idfsReference = v.[sflHC_FacilityWherePatientFSC_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000111) [ref_sflHC_CaseProgressStatus] ON 
	[ref_sflHC_CaseProgressStatus].idfsReference = v.[sflHC_CaseProgressStatus_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000150) [ref_sflHC_ReasonForNotCollectingSample] ON 
...
LEFT JOIN fnReferenceRepair('kk', 19000045) [ref_sflHCDiagnosisHistory_Organization] ON 
	[ref_sflHCDiagnosisHistory_Organization].idfsReference = v.[sflHCDiagnosisHistory_Organization_ID] 
LEFT JOIN fnReferenceRepair('kk', 19000046) [ref_sflHCSample_CollectedByOffice] ON 
	[ref_sflHCSample_CollectedByOffice].idfsReference = v.[sflHCSample_CollectedByOffice_ID] 
	
LEFT JOIN fnGisExtendedReferenceRepair('kk', 19000003) [ref_GIS_sflHC_ResidenceRegion] ON 
	[ref_GIS_sflHC_ResidenceRegion].idfsReference = v.[sflHC_ResidenceRegion_ID] 
LEFT JOIN fnGisExtendedReferenceRepair('kk', 19000002) [ref_GIS_sflHC_ResidenceRayon] ON 
	[ref_GIS_sflHC_ResidenceRayon].idfsReference = v.[sflHC_ResidenceRayon_ID] 
LEFT JOIN fnGisExtendedReferenceRepair('kk', 19000004) [ref_GIS_sflHC_ResidenceSettlement] ON 
