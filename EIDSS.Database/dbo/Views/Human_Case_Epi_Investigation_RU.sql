...
create VIEW [dbo].[Human_Case_Epi_Investigation_RU]
AS
...
LEFT JOIN fnReferenceRepair('ru', 19000046) [ref_sflHC_InvestigatedByOffice] ON 
	[ref_sflHC_InvestigatedByOffice].idfsReference = v.[sflHC_InvestigatedByOffice_ID] 
LEFT JOIN fnReferenceRepair('ru', 19000100) [ref_sflHC_RelatedToOutbreak] ON 
...
LEFT JOIN fnGeoLocationTranslation('ru') [ref_GL_sflHCContact_Information] ON 
	[ref_GL_sflHCContact_Information].idfGeoLocation = v.[sflHCContact_Information_ID] 
LEFT JOIN fnReferenceRepair('ru', 19000014) [ref_sflHCContact_Relation] ON 
