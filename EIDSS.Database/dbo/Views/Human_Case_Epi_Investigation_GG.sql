...
CREATE VIEW [dbo].[Human_Case_Epi_Investigation_GG]
AS
...
LEFT JOIN fnReferenceRepair('ka', 19000046) [ref_sflHC_InvestigatedByOffice] ON 
	[ref_sflHC_InvestigatedByOffice].idfsReference = v.[sflHC_InvestigatedByOffice_ID] 
LEFT JOIN fnReferenceRepair('ka', 19000100) [ref_sflHC_RelatedToOutbreak] ON 
...
LEFT JOIN fnGeoLocationTranslation('ka') [ref_GL_sflHCContact_Information] ON 
	[ref_GL_sflHCContact_Information].idfGeoLocation = v.[sflHCContact_Information_ID] 
LEFT JOIN fnReferenceRepair('ka', 19000014) [ref_sflHCContact_Relation] ON 
