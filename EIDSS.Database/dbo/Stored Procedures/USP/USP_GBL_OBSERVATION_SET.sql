...
-- Doug Albanese	4/10/2020	Bypassed a check that was crossing idfsformTemplate with idfsformType, which is wrong.
-- ============================================================================
...
BEGIN
    ...
    BEGIN TRY
		...
		IF NOT EXISTS (SELECT * FROM dbo.tlbObservation WHERE idfObservation = @idfObservation AND intRowStatus = 0)
			BEGIN
			Declare @ff bigint
			--WHAT???? The idfsFormTemplate is used as a Form Type??? BIG MISTAKE
			--Set @ff = (Select top (1) idfsFormTemplate from dbo.ffFormTemplate where idfsFormType = @idfsFormTemplate and intRowStatus = 0)
