...
-- LAMONT MITCHELL	11/23/21 Created New From Old with updates to output
-- Testing code:
...
--      @idfObservation 
--*************************************************************
CREATE PROCEDURE [dbo].[USP_VCTS_VECT_RW_SET]
(
...
DECLARE	@ReturnMessage					NVARCHAR(max) = 'SUCCESS' 
	...
	--Create a temp table to suppress the return values from secondary stored procedures
	--The API fails to handle mutiple returns sets from a SP
...
BEGIN
	BEGIN TRY
		...
		IF @@TRANCOUNT > 0 AND @ReturnCode =0
			COMMIT
   SELECT @ReturnCode 'ReturnCode', @ReturnMessage 'ReturnMessage',@idfVector 'idfVector',  @strVectorID 'strVectorID',@idfDetailedLocation 'idfDetailedLocation'
	END TRY
