...
EXEC spValidateKeys 'tlbHuman', NULL, NULL
*/

CREATE PROCEDURE [dbo].[usp_ValidateKeys]
...
IF (SELECT COUNT(*) FROM tstCheckTables tct JOIN tstCheckConstraints tcc ON tcc.idfCheckTable = tct.idfCheckTable WHERE strRootTableName = @TableName AND ISNULL(tct.strRootObjectType, '') = ISNULL(@RootObjectName, '')) > 0
BEGIN
	...
	IF @TableName1 <> @TableName2 
		AND ISNULL(@AdditionalJoin, '') NOT LIKE 'JOIN ' + @TableName2 + '%'	/*дает возможность не использовать алиасы, а значит гибче строить условия отбора с использованием имен таблиц*/
	BEGIN
	...
	END
	ELSE /*зашиваем жестко на алиасы*/
		BEGIN 
