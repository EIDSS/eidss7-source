...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			...
			FROM ffParameter p
			INNER JOIN dbo.ffParameterType pt ON p.idfsParameterType = pt.idfsParameterType
		...
		IF (
				...
				OR (@idfsFormType = 10034021)
				OR (@idfsFormType = 10034022)
				OR (@idfsFormType = 10034023)
				OR (@idfsFormType = 10034024)
				)
			SET @aggParam = 1
		ELSE
			SET @aggParam = 0
		...
		BEGIN
			...
			INNER JOIN dbo.ffParameterFixedPresetValue fpv ON p.idfsParameterType = fpv.idfsParameterType
				AND fpv.[intRowStatus] = 0
			INNER JOIN dbo.FN_GBL_Reference_List_GET(@LangID, 19000069 /*'rftParametersFixedPresetValue'*/) fr ON fpv.idfsParameterFixedPresetValue = fr.idfsReference
			WHERE p.idfsParameter = @idfsParameter
			ORDER BY fr.intOrder ASC
				,fr.LongName ASC
		END
		ELSE IF (@idfsReferenceType IS NOT NULL)
		BEGIN
