...
(
	...
	,@idfsStatisticDataType BIGINT =NULL
)
AS
BEGIN
BEGIN TRY
		...
		IF( @advancedSearch IS NOT NULL )
			...
			WHERE sdt.[intRowStatus] = 0 AND 
			(
				sdtpt.name LIKE + '%' + @advancedSearch + '%' OR
				sdtbr.strDefault LIKE + '%' + @advancedSearch + '%' OR
				sdtbr.name LIKE + '%' + @advancedSearch + '%' OR
				sptbr.name LIKE + '%' + @advancedSearch + '%' OR
				satbr.name LIKE + '%' + @advancedSearch + '%' 
			)AND (idfsStatisticDataType = @idfsStatisticDataType OR  @idfsStatisticDataType  IS NULL);
		ELSE
			INSERT INTO @T		
			SELECT
				sdt.[idfsStatisticDataType],
