...
--*************************************************************************
-- Name 				: report.USP_REP_VET_CaseReportAZ
-- DescriptiON			: Select data for Avian Test report.
--          
-- Author               : Srini Goli
-- Revision History
...
EXEC report.USP_REP_VET_CaseReportAZ @LangID=N'en', @FromYear = 2012, @ToYear = 2016
EXEC report.USP_REP_VET_CaseReportAZ 'ru',2012,2016
*/

CREATE  PROCEDURE [Report].[USP_REP_VET_CaseReportAZ]
    (
...
FROM report.FN_GBL_ReferenceRepair_GET(@LangID, 19000132) fr
...
, VetCase4 AS ( /*подсчитываем количество кейсов, сэмплов, энималов по каждой паре диагноз-спишес*/
	...
	FROM VetCase3 vc3
	JOIN report.FN_GBL_ReferenceRepair_GET(@LangID, 19000086) frr ON	/*Species List*/
...
, AllCase2 AS ( /*суммируем числовые показатели отобранных кейсов по диагнозам и спишес-тайпам*/
	...
	JOIN dbo.FN_GBL_DiagnosisRepair(@LangID, 96, NULL) fdr ON
...
, NotDeletedDiagnosis AS (
	...
	LEFT JOIN trtDiagnosis d_actual
		JOIN report.FN_GBL_ReferenceRepair_GET(@LangID, 19000019) r_d_actual ON
...
, AllCase3 AS ( /*добавляем к информации, полученной из кейсов пустые дигнозы, которые не попали в выборку по кейсам
	...
	LEFT JOIN report.FN_GBL_ReferenceRepair_GET(@LangID, 19000019) r_d_actual ON
...
FROM (
	...
	SELECT
		...
		,	IsNull	(	(	select	max(IsNull(r_st.intOrder, 0))
							from	report.FN_GBL_ReferenceRepair_GET('en', 19000086) r_st /*Species List*/
						), 0
					) + 1
					
	FROM AllCase3
) x
JOIN report.FN_GBL_ReferenceRepair_GET(@LangID, 19000019) fdr ON	/*Diagnosis*/
